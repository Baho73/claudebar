...
//   LAST_CHANGE: v1.15.0 - M-MAIL: ключ maildonecmd= (команда «входящие разобраны», {dir} -> папка в кавычках; пусто = пункт меню серый).
//   v1.14.0 - M-MAIL: ключ sessioncmd= (команда запуска сессии агента в папке проекта, {dir} -> путь в кавычках; дефолт «wt.exe -d {dir} claude»).
...
// Команда «входящие разобраны» (пункт меню): роутер переносит .inbox/*.md в .inbox/done/ и
// снимает значок. {dir} -> папка проекта в кавычках. Пусто -> пункт меню серый. Ключ: maildonecmd=
...
pub struct Config {
    ...
    pub voice_streaming: bool, // инкрементально распознавать длинную диктовку во время записи (ключ voicestreaming=, деф. false) — Phase-24
...
fn parse_ini(text: &str) -> ParsedIni {
    ...
    let mut voice_streaming = false; // деф. ВЫКЛ: стриминг длинной диктовки (новая, рискованная)
    ...
    for line in text.lines() {
        ...
        } else if let Some(v) = line.strip_prefix("voicestreaming=") {
            voice_streaming = v.trim() == "1";
...
impl Config {
    pub fn load(cfg_path: PathBuf) -> Config {
        ...
        Config {
            ...
            voice_streaming: p.voice_streaming,
            cfg_path,
        }
    }
    ...
    pub fn serialize(&self, pos: Option<(i32, i32)>) -> String {
        ...
        out += &format!("voicestreaming={}\n", if self.voice_streaming { 1 } else { 0 });
...
mod tests {
    ...
    #[test]
    fn voice_always_on_roundtrip_default_false() {
        // дефолт ВЫКЛ (нет ключа / =0 -> микрофон только на запись)
        assert!(!parse_ini("font=X\t14\t400").voice_always_on);
        assert!(!parse_ini("voicealwayson=0").voice_always_on);
        let mut c = cfg(vec![]);
    ...
    #[test]
    fn voice_streaming_roundtrip_default_false() {
        // дефолт ВЫКЛ (нет ключа / =0 -> стриминг выключен)
        assert!(!parse_ini("font=X\t14\t400").voice_streaming);
        assert!(!parse_ini("voicestreaming=0").voice_streaming);
        let mut c = cfg(vec![]);
        assert!(!c.voice_streaming);
        c.voice_streaming = true; // включили стриминг
        assert!(parse_ini(&c.serialize(None)).voice_streaming); // round-trip
    }

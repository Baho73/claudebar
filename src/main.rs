...
//   PURPOSE: Точка входа Win32 + оконная процедура + оркестрация модулей панели (окна, недавние, поиск, сигналы, голос).
//   SCOPE: создание окна (always-on-top, tool-window), цикл сообщений, диспетчеризация WndProc;
//          опрос (окна/сигналы/недавние) по таймеру, анимационный таймер, глобальный хоткей;
//          фоновые потоки (индексация, карта путей документов); оркестрация поиска, голоса (M-VOICE) и вставки (M-PASTE).
...
//
// START_MODULE_MAP
//   wndproc              - оконная процедура: paint, клики/hover, таймеры, WM_APP_* (поиск/голос); меню и поле поиска делегируются в M-MENU/M-SEARCHBOX
...
//   run_live_search / commit_search_enter - живой BM25 (M-SEARCH); вызывается из M-SEARCHBOX по EN_CHANGE/Enter
//   spawn_index / spawn_files_index / spawn_doc_paths - фоновые потоки: авто-индексация чатов и файлов, карта путей документов
//   update_anim_timer    - аним-таймер (змейка квадратов / полоса голоса) только при активности
//   hotkey / voice       - глобальный хоткей -> voice.toggle; WM_APP_VOICE_DONE -> take_result + paste_text; WM_APP_STREAM_PARTIAL -> on_partial
//   menu_link_for / url_to_path / percent_decode / explorer_folder_path - цель «ссылка/проводник» (COM, побайтный percent-decode)
//   make_font / rebuild_fonts / create_window / main - инициализация окна, шрифтов, цикл сообщений
// END_MODULE_MAP
//
// START_CHANGE_SUMMARY
//   LAST_CHANGE: v2.0.0 - рефактор god-object завершён (FPF D-5): меню и обработчик команд -> M-MENU. Ранее: App/APP -> M-STATE, prompt -> M-PROMPT, tooltip -> M-TOOLTIP, поле поиска+история -> M-SEARCHBOX. Поведение не менялось (102/102 на каждом шаге).
//   v1.0.0 - FPF D-23: добавлен MODULE_CONTRACT/MODULE_MAP (main.rs был единственным модулем без разметки).
// END_CHANGE_SUMMARY
...
use tooltip::{arm_tip, create_tooltip, show_tooltip};
...
// меню и обработчик команд — M-MENU.
use menu::{handle_command, show_menu, show_settings_menu, MENU_ACTIVE};
...
pub(crate) fn spawn_files_index(hwnd: HWND) {
...
// copy_to_clipboard / open_in_explorer_select вынесены в M-MENU (src/menu.rs) — их зовут только команды меню.
...
// ---------- контекст-меню, настройки и обработчик команд вынесены в M-MENU (src/menu.rs) ----------
...
pub(crate) fn rebuild_fonts(app: &mut App) {
    unsafe {
        let _ = DeleteObject(app.font_main);
        let _ = DeleteObject(app.font_small);
    }

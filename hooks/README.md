...
ClaudeBar анимирует «...» справа от названия проекта. `PostToolUse` обновляет mtime `.busy` на
каждом инструменте (keep-alive) — точки держатся всю длинную задачу. `Stop` удаляет `.busy`
(и пишет `.signal` звоночка).

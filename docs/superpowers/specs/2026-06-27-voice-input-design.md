...
| **M-TRANSFORM** | CORE | `clean_whisper(text)` — хардкод-чистка; + конфиг-шов под внешний API (переформ./перевод, default off) | `clean_whisper`, `select_mode`, `build_ext_request`/`parse_ext` |
...
language       = ru
hotwords       =                            # «горячие слова»: имена, термины, бренды (через пробел)
initial_prompt =                            # затравка-контекст для орфографии
...
`hotwords` и `initial_prompt` из ini прокидываются M-STT в `/transcribe`. Никакого отдельного endpoint —
faster-whisper их поддерживает. Редактирование — руками в ini (UI настроек — позже).
